 ...
 * 
 * <Name:- Pratik Shekhar - Drexel ID: 13602117>
 */
#include <stdio.h>
#include <stdlib.h>
#include <unistd.h>
        ...
        while (*arg){
            if (!strcmp(*arg, "<")){
                ...
                *arg = NULL;
                ...
                if ((outputfd = open(*(arg + 1), O_WRONLY | O_CREAT | O_TRUNC, 0666)) < 0)
...
void sigchld_handler(int sig) 
{
   ...
   while((pid=waitpid(-1,&status,WNOHANG|WUNTRACED))>0){
       ...
       if(WIFSTOPPED(status)){
           ...
           job->state = 3; //changing the state of stopped jobs to ST i.e. 3
       }
